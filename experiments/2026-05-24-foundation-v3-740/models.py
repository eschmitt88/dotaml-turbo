...
class FoundationTransformer(nn.Module):
    def __init__(self, vocab_size: int, embed_dim: int, d_model: int,
        ...
        # v3 change: duration is now a scalar regression (SmoothL1 on log(seconds+1))
        # in train.py. n_dur_buckets is retained in init signature for back-compat
        # and used in train.py for the POST-HOC bucket-top1-acc diagnostic only.
    ...
    def _encode_tokens(self, hero_ids: torch.Tensor,
                        ...
                        patch_mask: torch.Tensor | None = None) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    def forward(self, hero_ids: torch.Tensor,
        ...
        out = {
            ...
            "dur":  self.dur_head(dur_h).squeeze(-1),            # [B] (v3: scalar regression on log-seconds)
...
def build_model(hp: dict, vocab_size: int, n_player_feats: int,
                ...
                use_lobby_token: bool | None = None) -> FoundationTransformer:
