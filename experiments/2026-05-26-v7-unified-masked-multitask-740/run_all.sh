#!/usr/bin/env bash
# Full-run script for v7-unified-masked-multitask-740.
#
# Sequential pipeline (each step gates the next on rc=0):
#   1) smoke (1 epoch, 50k rows) -- verifies plumbing + probe suite
#   2) v7_unified full training (max 25 epochs, ~10-15h GPU)
#
# `python -u` for unbuffered stdout under nohup.
...
#
# No data build phase. Reuses extended player_features + rich_cols
# parquets from foundation-v3-740 verbatim.
#
# Main agent invokes via:
#   nohup bash experiments/2026-05-26-v7-unified-masked-multitask-740/run_all.sh \
#     > experiments/2026-05-26-v7-unified-masked-multitask-740/full_run.log 2>&1 &

set -u
cd "$(dirname "$0")/../.."  # project root

PY=".venv/bin/python -u"
EXP="experiments/2026-05-26-v7-unified-masked-multitask-740"
ABL="v7_unified"
MAX_RETRIES=3

echo "===== v7-unified-masked-multitask-740 run_all START $(date -Iseconds) ====="
...
rc_v7=$?
echo "[$(date -Iseconds)] STEP 2/2 DONE rc=$rc_v7"

if [ "$rc_v7" -eq 0 ]; then
  cp "$EXP/metrics_${ABL}.json" "$EXP/metrics.json"
  AUC=$($PY -c "import json; print(json.load(open('$EXP/metrics.json'))['val_auc_pure_pregame'])")
  echo "metrics.json <- $ABL (pure_pregame_val_auc=$AUC)"
fi

echo "===== v7-unified-masked-multitask-740 run_all DONE rc=$rc_v7 $(date -Iseconds) ====="
exit "$rc_v7"
