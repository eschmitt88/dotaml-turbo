"""Train MinimalTransformerWithFeaturesAndPlayerEmbedding for one ablation.

Two ablations:
  --ablation baseline_extended_clean : Transformer + features only (no embedding).
                                       Sanity replication of cleanup-740 (~0.6477).
  --ablation with_player_embedding   : PRIMARY. Add learned per-account embedding.

Per-trial subprocess isolation is the workaround for the Blackwell+torch 2.9-2.12
DataLoader GC segfault (docs/decisions/0001-per-trial-subprocess-isolation.md).

Math SDP backend is forced at module load to avoid sm_120 flash/mem-eff
attention crashes (transformer-hp-sweep-740 workaround).
"""
...
from data import load_train_val, load_vocab  # noqa: E402
...
def evaluate(model, loader, device, autocast_dtype, use_features: bool,
    ...
    total_loss = 0.0
    ys, ps = [], []
    for hero_ids, player_feats, account_idx, y in loader:
        hero_ids = hero_ids.to(device, non_blocking=True)
        player_feats = player_feats.to(device, non_blocking=True)
        account_idx = account_idx.to(device, non_blocking=True)
        y = y.to(device, non_blocking=True)
        with torch.autocast(device_type=device.type, dtype=autocast_dtype,
                            enabled=autocast_dtype is not None):
            logits = model(hero_ids,
                           player_feats if use_features else None,
                           account_idx if use_player_embedding else None)
...
def build_param_groups(model: nn.Module, base_wd: float, embed_wd: float) -> list[dict]:
    """Two parameter groups: player_embedding params get embed_wd, rest get base_wd.

    When use_player_embedding=False the player_embedding params don't exist;
    we emit only the base group.
    """
    emb_params = []
    other_params = []
    ...
    groups = [{"params": other_params, "weight_decay": base_wd, "name": "other"}]
    if emb_params:
        groups.append({"params": emb_params, "weight_decay": embed_wd,
...
def train_model(model, train_ds, val_ds, hp: dict, max_epochs: int,
                ...
                embed_wd: float, mixed_precision: bool = True,
    ...
    groups = build_param_groups(model, weight_decay, embed_wd)
    ...
    for epoch in range(max_epochs):
        ...
        loss_sum = 0.0
        ep_t0 = time.time()
        for hero_ids, player_feats, account_idx, y in train_loader:
            hero_ids = hero_ids.to(device, non_blocking=True)
            player_feats = player_feats.to(device, non_blocking=True)
            account_idx = account_idx.to(device, non_blocking=True)
            y = y.to(device, non_blocking=True)
        ...
        val_eval = evaluate(model, val_loader, device, autocast_dtype,
                            use_features, use_player_embedding)
...
def coverage_bucket_diagnostics(val_ds, y_val: np.ndarray, p_val: np.ndarray,
                                feat_names: list[str],
                                anon_idx: int, rare_idx: int) -> dict:
    """Bucket val by mean p*_n_games_log1p across 10 slots (low/med/high tercile).
    Report AUC, in-vocab fraction, anon share, rare share per bucket.
    """
    ...
    acct_np = val_ds.account_idx.numpy()
    in_vocab_per_slot = (acct_np >= 2)
    anon_per_slot = (acct_np == anon_idx)
    rare_per_slot = (acct_np == rare_idx)
    bucket_info = {}
    for b, name in [(0, "low"), (1, "medium"), (2, "high")]:
        mask = buckets == b
        n = int(mask.sum())
        if n < 100:
            bucket_info[name] = {"n": n, "val_auc": None,
        ...
        bucket_info[name] = {
            ...
            "in_vocab_frac": float(in_vocab_per_slot[mask].mean()),
            "anon_share": float(anon_per_slot[mask].mean()),
            "rare_share": float(rare_per_slot[mask].mean()),
        }
    return {
        "quantile_edges_log1p": [float(q33), float(q67)],
        "buckets": bucket_info,
        "overall": {
            "in_vocab_frac": float(in_vocab_per_slot.mean()),
            "anon_share": float(anon_per_slot.mean()),
            "rare_share": float(rare_per_slot.mean()),
        },
    }
...
def main() -> int:
    ...
    ap.add_argument("--ablation", required=True,
                    choices=["baseline_extended_clean", "with_player_embedding"])
    ap.add_argument("--smoke", action="store_true",
                    help="Use 50k train / 5k val and 1-epoch cap (pipeline test only).")
    ap.add_argument("--metrics-suffix", default="",
                    help="Suffix appended to metrics filename. e.g. '_baseline_extended_clean'.")
    args = ap.parse_args()
    ...
    if not vocab_path.exists():
        sys.exit(f"REFUSED: vocab not found at {vocab_path}. Run build_vocab.py first.")
    vocab, vocab_meta = load_vocab(vocab_path)
    print(f"Loaded vocab: vocab_size={vocab_meta.get('vocab_size'):,}  "
          f"coverage={vocab_meta.get('coverage_of_topN_over_non_anon'):.3f}  "
          f"anon_share={vocab_meta.get('anonymous_share'):.3f}")

    # Sidecar dir + filenames.
    side_cfg = cfg["account_sidecar"]
    sidecar_dir = PROJECT_ROOT / side_cfg["out_dir"]
    if args.smoke and (sidecar_dir / side_cfg["smoke_train_filename"]).exists():
        sidecar_train_name = side_cfg["smoke_train_filename"]
        sidecar_val_name = side_cfg["smoke_val_filename"]
    else:
        sidecar_train_name = side_cfg["train_filename"]
        sidecar_val_name = side_cfg["val_filename"]

    # Load data.
    print(f"Ablation: {args.ablation} (use_features={use_features}, "
          f"use_player_embedding={use_player_embedding})")
    t0 = time.time()
    n_target = int(cfg["train_subset_size"])
    if args.smoke:
        train_ds, val_ds, meta = load_train_val(
            seed=seed, n_target=n_target, feat_names=feat_names,
            source_dir=source_dir, sidecar_dir=sidecar_dir,
            sidecar_train_name=sidecar_train_name,
            sidecar_val_name=sidecar_val_name,
            vocab=vocab, anon_idx=int(pe["anon_idx"]), rare_idx=int(pe["rare_idx"]),
            splits=splits, smoke=True,
    ...
    print(f"  in_vocab frac (train) {meta['train_in_vocab_frac']:.3f}  "
          f"(val) {meta['val_in_vocab_frac']:.3f}  "
          f"anon (val) {meta['val_anon_share']:.3f}")
    ...
    model = build_model(
        mhp,
        hero_vocab_size=int(cfg["hero"]["vocab_size"]),
        n_player_feats=n_player_feats,
        use_features=use_features,
        use_player_embedding=use_player_embedding,
        player_vocab_size=int(vocab_meta["vocab_size"]),
        player_embed_dim=int(pe["embed_dim"]),
        player_init_std=float(pe.get("init_std", 0.02)),
    )
    ...
    max_epochs = int(opt_cfg["max_epochs"]) if not args.smoke else int(cfg["transformer_smoke"]["max_epochs"])
    ...
    embed_wd = float(pe["weight_decay_embedding"])
    ...
    cov_info = None
    try:
        cov_info = coverage_bucket_diagnostics(
            val_ds, y_val, p_val, feat_names,
            int(pe["anon_idx"]), int(pe["rare_idx"]),
        )
    ...
    dirty_anchor = float(anchors.get("transformer_plus_features_extended_740_val_auc", 0.6477298))
    clean_anchor = float(anchors.get("upstream_data_cleanup_740_clean_val_auc", 0.6477054))
    target = float(anchors.get("proposal_target_val_auc", 0.6497))
    band_lo = float(anchors.get("equality_band_lower", 0.6467))
    band_hi = float(anchors.get("equality_band_upper", 0.6487))

    equality_band_check = None
    if not args.smoke:
        in_band = bool(band_lo <= tr.best_val_auc <= band_hi)
        equality_band_check = {
            "band_lower": band_lo,
            "band_upper": band_hi,
            "this_val_auc": float(tr.best_val_auc),
            "delta_vs_clean_anchor": float(tr.best_val_auc - clean_anchor),
            "equality_band_holds": in_band,
        }
    ...
    metrics = {
        ...
        "optim_hp": {"batch_size": hp["batch_size"], "lr": hp["lr"],
                     ...
                     "weight_decay_embedding": embed_wd,
                     "mixed_precision": bool(opt_cfg["mixed_precision"])},
        "param_counts": pc,
        ...
        "train_in_vocab_frac": meta["train_in_vocab_frac"],
        "val_in_vocab_frac": meta["val_in_vocab_frac"],
        "train_anon_share": meta["train_anon_share"],
        "val_anon_share": meta["val_anon_share"],
        "anchors": anchors,
        "delta_vs_transformer_plus_features_extended_740_dirty": float(tr.best_val_auc - dirty_anchor),
        "delta_vs_upstream_data_cleanup_740_clean": float(tr.best_val_auc - clean_anchor),
        "delta_vs_proposal_target": float(tr.best_val_auc - target),
        "coverage_bucket_val_auc": cov_info,
        "calibration": cal,
        "equality_band_check": equality_band_check,
    }
