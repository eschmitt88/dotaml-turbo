"""Train LightGBM player-features-prepatch-740.

Mirrors player-features-740/train.py exactly except:
  - reads from data/snapshots/.../processed/player_features_prepatch/
    instead of player_features/
  - adds history-source-breakdown diagnostic to metrics.json:
    per coverage bucket, mean(n_games_prepatch / max(1, n_games_total))
    across the 10 player-slots per match.

Ablations:
  --ablation heroes_only           : sparse hero one-hot + side bit ONLY
                                     (must reproduce plateau-baseline-740 within 0.001)
  --ablation features_only         : dense player feats ONLY
  --ablation heroes_plus_features  : both (default)
"""
...
def main() -> int:
    ...
    metrics = {
        ...
        "delta_vs_proposal_target_val_auc": val_auc - float(
            anchors.get("proposal_target_val_auc", 0.6277)),
