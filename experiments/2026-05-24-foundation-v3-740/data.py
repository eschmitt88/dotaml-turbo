...
class FoundationDataset(Dataset):
    """Foundation-MVP dataset. Yields:
      ...
      y_win:       scalar f32
      y_dur:       scalar f32   (v3: log(duration_seconds + 1.0))
      y_dur_bucket: scalar long (post-hoc bucket idx — diagnostic only, not loss)
    ...
    def __init__(self, hero_ids: np.ndarray, player_feats: np.ndarray,
        ...
        # v3: y_dur is now a log-seconds float scalar (regression target).
        self.y_dur = torch.tensor(y_dur, dtype=torch.float32).contiguous()
        # Post-hoc bucket label kept for diagnostic top1-acc comparison vs
        # multitask-740 / iso_pmae anchors (not used in loss).
...
def load_train_val(seed: int, n_target: int, feat_names: list[str],
    ...
    (h_tr_k, pf_tr_k, y_win_tr, y_dur_tr, y_durb_tr,
     items_tr, y_kda_tr, y_gpm_tr, y_hd_tr, mu, sd) = \
    ...
    (h_va_k, pf_va_k, y_win_va, y_dur_va, y_durb_va,
     items_va, y_kda_va, y_gpm_va, y_hd_va, _, _) = \
        _build_foundation_targets(
