"""Train FoundationTransformer for one ablation in foundation-mvp-740.

Three ablations selectable via --ablation:
  - baseline_multitask_repro : 5M model, multi-task, NO PMAE / NO patch token /
                                 NO team-team bias / NO UW-SO (fixed alpha).
                                 Anchors the scaling lift.
  - foundation_mvp          : PRIMARY. Full design.
  - foundation_no_patch_token: PRIMARY minus the patch token (sanity).
...
from mae import PMAEMasker, pmae_reconstruction_loss  # noqa: E402
    ...
    for epoch in range(max_epochs):
        ...
        for batch in train_loader:
            ...
            with torch.autocast(device_type=device.type, dtype=autocast_dtype,
                ...
                # PMAE reconstruction: teacher pass = unmasked.
                l_mae = torch.tensor(0.0, device=device)
                if use_pmae and mask_out is not None:
                    with torch.no_grad():
                        teacher_out = model(hero_ids, pf if use_features else None,
                                              patch_id=patch_id if use_patch_token else None,
                                              hero_mask=None, patch_mask=None)
                    teacher_enc = teacher_out["encoded"].float()
                    student_enc = out["encoded"].float()
                    l_mae = pmae_reconstruction_loss(
...
def main() -> int:
    ...
    ap.add_argument("--ablation", required=True,
                    choices=["baseline_multitask_repro", "foundation_mvp",
