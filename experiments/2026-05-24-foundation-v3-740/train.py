"""Train FoundationTransformer for foundation-v3-740.

v3 = v2 component-isolation code MINUS UW-SO PLUS:
  - hand-tuned alpha weights (multitask-740 values 1.0/0.15/0.3/0.1)
  - duration head switched from 8-bucket CE to 1-scalar SmoothL1 on log(seconds+1)
  - canonical hero sort + (team, team) attention bias + PMAE EMA-teacher kept
  - patch_id token (now meaningful on extended cross-patch corpus)
  - extended training data (Aug 2025 -> Feb 2026) via build_features_extended.py

Single primary ablation: `foundation_v3`. No UW-SO branch reachable.
...
# v3: UWSO no longer imported (saboteur per iso_uwso). loss.py retained as
# legacy module but no public class is used here.
...
def _eval_multitask(model, loader, device, autocast_dtype, use_features: bool,
    ...
    """v3: duration head is a single-scalar regression on log(seconds+1).

    Returns dur_pred_log (regression scalar predictions) AND y_dur_bucket
    (post-hoc bucket labels from data.py) for top1-acc diagnostic.
    """
    model.eval()
    bce_sum = nn.BCEWithLogitsLoss(reduction="sum")
    bce_item = nn.BCEWithLogitsLoss(reduction="sum")
    n = 0
    tot_w = tot_d_l1 = tot_i = 0.0
    tot_d_sq = 0.0           # for MSE / Pearson
    ...
    with torch.no_grad():
        for batch in loader:
            (hero_ids, pf, patch_id, y_win, y_dur, y_dur_bucket, y_item,
            ...
            # y_dur_bucket stays on CPU for the digitize post-hoc compare.
    ...
    val_losses = {
        ...
        "dur_smoothl1_per_row_log_seconds": tot_d_l1 / max(n, 1),
        "dur_mse_log_seconds":              tot_d_sq / max(n, 1),
        "dur_mae_log_seconds":              float(np.mean(np.abs(
    ...
    return {
        ...
        "dur_p_log": np.concatenate(ps_dur_log),
...
def post_hoc_duration_bucket_top1(dur_p_log: np.ndarray,
    ...
    """Post-hoc bucket-top1-acc on the regression head, for direct comparison
    to prior anchors' dur_top1_acc. Convert predicted log-seconds -> seconds,
    bucket via the same edges, compare to ground-truth bucket label.
    """
    if len(dur_p_log) == 0:
        return float("nan")
...
def per_patch_val_auc(y_win: np.ndarray, p_win: np.ndarray,
    ...
    """Group val rows by patch_id and report AUC per group (v3 diagnostic
    for cross-patch generalization)."""
    out: dict = {}
...
def train_one_ablation(model, train_ds, val_ds, hp: dict, max_epochs: int,
    ...
    v3: UW-SO is dropped entirely (saboteur per iso_uwso). Hand-tuned alphas
    only. Duration is regression (SmoothL1 on log-seconds).
    """
    ...
    # Loss weighting -- v3 uses hand-tuned alpha weights only (multitask-740 values).
    ...
    opt = torch.optim.Adam(list(model.parameters()), lr=lr, weight_decay=wd)
    ...
    # v3: duration loss is SmoothL1 on log(seconds+1).
    ...
    for epoch in range(max_epochs):
        ...
        for batch in train_loader:
            ...
            y_win = y_win.to(device, non_blocking=True)
            y_dur = y_dur.to(device, non_blocking=True)
            # y_dur_bucket NOT used in training loss (post-hoc diagnostic only).
            y_item = y_item.to(device, non_blocking=True)
            y_kda = y_kda.to(device, non_blocking=True)
            y_gpm = y_gpm.to(device, non_blocking=True)
            y_hd = y_hd.to(device, non_blocking=True)
            ...
            with torch.autocast(device_type=device.type, dtype=autocast_dtype,
                ...
                # v3: duration regression on log(seconds+1).
                ...
                # v3: supervised combined loss = hand-tuned alpha-weighted sum (no UW-SO).
                omega = fixed_alpha
        ...
        # v3: duration regression diagnostics + post-hoc bucket-top1-acc.
        dur_acc_bucket = post_hoc_duration_bucket_top1(
            eval_out["dur_p_log"], eval_out["dur_y_bucket"], duration_bucket_edges
        )
        dur_pearson = pearson_corr(eval_out["dur_p_log"], eval_out["dur_y_log"])
        dur_mae_log = float(vl["dur_mae_log_seconds"])
        # Per-patch val_auc diagnostic.
        ...
        print(f"  epoch {epoch+1}/{max_epochs}  "
              ...
              f"dur_mae_log={dur_mae_log:.4f} dur_bucket_top1={dur_acc_bucket:.4f} "
        ...
        history.append({
            ...
            "val_dur_smoothl1_log_seconds": float(vl["dur_smoothl1_per_row_log_seconds"]),
            "val_dur_mse_log_seconds":      float(vl["dur_mse_log_seconds"]),
            "val_dur_mae_log_seconds":      float(dur_mae_log),
            "val_dur_pearson_log":          float(dur_pearson),
            "val_dur_bucket_top1_posthoc":  float(dur_acc_bucket),
        ...
        if val_win_loss < best_val_loss - 1e-6:
            ...
            best_eval = {"win": m_w,
                          ...
                          "dur_bucket_top1_posthoc": dur_acc_bucket,
                          "per_patch_val_auc": patch_aucs,
...
def main() -> int:
    ...
    # v3: UW-SO is permanently off; cf. iso_uwso saboteur diagnosis.
    use_uw_so = False
    if bool(ab_spec.get("use_uw_so", False)):
        sys.exit("REFUSED: v3 does not support use_uw_so=True (saboteur per iso_uwso).")
    ...
    print(f"Ablation: {args.ablation} "
          ...
          f"pmae={use_pmae} uw_so=False[v3-locked])")
    t0 = time.time()
