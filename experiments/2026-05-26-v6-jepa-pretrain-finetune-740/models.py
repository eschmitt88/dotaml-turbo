"""FoundationTransformer for v6-jepa-pretrain-finetune-740.

Forked from experiments/2026-05-26-v5-pretrain-finetune-740/models.py.

vs v5 — the architecture is IDENTICAL. We keep the same input
projections + per-group mask tokens + (unused-in-v6) reconstruction
heads + decoder + multi-task heads. The class is renamed V5 -> V6 for
clarity; state-dict keys are the same. v6's Phase 1 JEPA loss simply
doesn't flow gradients through the per-group reconstruction heads, so
they remain at xavier init throughout training. They're carried so the
state_dict matches v5's verbatim and so the small subtree could be
reused if a future variant wanted a hybrid JEPA+reconstruction loss.

The JEPA predictor MLP is NOT part of this module — it lives in
mae.py:JEPAPredictor and is constructed alongside the model in
train.run_pretrain(). Keeping it separate means the saved encoder
checkpoint does not include the predictor, so Phase 2A/2B can load
strict against a plain FoundationTransformerV6.
...
class FoundationTransformerV6(nn.Module):
    """v6 encoder: identical architecture to v5 (6 input groups + per-group
    learned mask tokens + pre-train reconstruction heads + v4-style multi-task
    decoder heads). v6 differs only in the Phase 1 loss form — JEPA latent-space
    prediction (via mae.JEPAPredictor) instead of v5's per-group raw-target
    reconstruction. The reconstruction heads remain on the module but are not
    trained or read in v6.
    """
...
def encoder_param_names(model: "FoundationTransformerV6") -> list[str]:
...
def build_model_v6(hp: dict, vocab_size: int, n_player_feats: int,
                    *, n_dur_buckets: int = 8,
                    item_vocab_size: int = 1,
                    use_team_team_bias: bool | None = None,
                    dur_loss_mode: str = "ce") -> "FoundationTransformerV6":
    return FoundationTransformerV6(
...
__all__ = ["FoundationTransformerV6", "build_model_v6", "count_params",
           "encoder_param_names",
