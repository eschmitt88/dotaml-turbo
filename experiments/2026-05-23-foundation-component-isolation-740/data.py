"""Data loading for foundation-mvp-740.

Forked from experiments/2026-05-20-rich-supervision-multitask-740/data.py.

Differences from parent:

1. CANONICAL SORT BY HERO_ID (per team) at load time. Removes lobby-order
   signal from player_slot (which is arbitrary in Turbo). Hero ascending
   within each team, radiant first then dire. After sorting, hero_ids[:5]
   are sorted-ascending radiant heroes and hero_ids[5:] are sorted-ascending
   dire heroes. player_feats are reordered in lockstep, and the per-slot
   aux/item targets ALSO reordered in lockstep.

2. patch_id derived from snapshot_id config (only patch 7.40 in the current
   window -> patch_id = 1 for all rows; the slot is wired so a wider window
   could fill it differently per-match without re-coding).

3. aux_targets split into THREE separate per-slot tensors (kda, gpm, hd) so
   the new task-as-token decoder can address each independently. Each is
   [N, 10, 1] -- kda is a single scalar per slot per match (just kills here;
   keeping `kda` as the name in case the head later sums kills+assists-deaths).

4. HCE date assertion stays. Defensive parquet readers reused verbatim.

5. Optional: when canonical_sort=True is the input invariant, the on-disk
   parquet ordering is irrelevant.
"""
...
class FoundationDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        return (self.hero_ids[idx], self.player_feats[idx], self.patch_ids[idx],
                self.y_win[idx], self.y_dur[idx], items,
...
def _build_foundation_targets(
    ...
    return (hero_ids_kept, player_feats_kept, y_win_kept, y_dur,
            items_per_slot, y_kda, y_gpm, y_hd,
...
def _patch_id_from_dates(dates: np.ndarray, default_patch_id: int = 1) -> np.ndarray:
    ...
    For the current 7.40-only window, every match is patch 7.40 -> patch_id = 1.
    Future extension: a {date -> patch} lookup table here.
    """
    return np.full(len(dates), default_patch_id, dtype=np.int64)
...
def load_train_val(seed: int, n_target: int, feat_names: list[str],
    ...
    h_tr_k, pf_tr_k, y_win_tr, y_dur_tr, items_tr, y_kda_tr, y_gpm_tr, y_hd_tr, mu, sd = \
    ...
    h_va_k, pf_va_k, y_win_va, y_dur_va, items_va, y_kda_va, y_gpm_va, y_hd_va, _, _ = \
        _build_foundation_targets(
    ...
    train_ds = FoundationDataset(h_tr_k, pf_tr_k, train_patch_ids_kept,
                                  y_win_tr, y_dur_tr, items_tr,
                                  y_kda_tr, y_gpm_tr, y_hd_tr, item_vocab_size)
    val_ds = FoundationDataset(h_va_k, pf_va_k, val_patch_ids_kept,
                                y_win_va, y_dur_va, items_va,
