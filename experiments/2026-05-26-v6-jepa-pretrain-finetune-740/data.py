"""Data loading for v5-pretrain-finetune-740.

Forked from experiments/2026-05-25-v4-iso-teambias-extended-740/data.py.

Changes vs v4:

1. Loads KDA / GPM / HD as INPUT scalars (per slot), in addition to the
   existing target scalars. Inputs are log1p-transformed (kda_input,
   hd_input) or raw (gpm_input), then standardized using train-fit
   per-feature mean/std. Returned tensors:
     scalar_inputs[10, 3] = (kda_in_std, gpm_in_std, hd_in_std)

   Where:
     kda_in_std = standardize(log1p((kills + assists) / max(deaths, 1)))
     gpm_in_std = standardize(gpm)
     hd_in_std  = standardize(log1p(hero_damage))

   These are the same SOURCE columns as y_kda/y_gpm/y_hd targets but
   with a different transform — the target side keeps v4's per-target
   z-score (kills, gpm, hero_damage independently standardized) for
   compatibility with v4 heads / loss.

2. Account-id sidecar plumbing is REMOVED — no player embedding in any
   v5 phase. account_idx and player_vocab are gone.

3. FoundationDataset __getitem__ now yields a 12-tuple including the new
   scalar_inputs[10, 3] tensor:
     (hero_ids, player_feats, patch_id, scalar_inputs,
      y_win, y_dur, y_dur_bucket, items,
      y_kda, y_gpm, y_hd)

All other v4 invariants preserved: HCE date assertion, defensive parquet
...
N_PLAYERS = 10
N_SCALAR_INPUTS = 3   # kda, gpm, hd inputs
...
class FoundationDataset(Dataset):
    """v5 dataset. Yields:
      ...
      scalar_inputs: [10, 3] f32   -- (kda_in_std, gpm_in_std, hd_in_std)
      y_win:       scalar f32
      y_dur:       scalar f32   (log(seconds+1))
      y_dur_bucket: scalar long (8-bucket)
      y_item:      [10, item_vocab_size] f32 multi-hot (materialized per __getitem__)
      y_kda:       [10] f32   (kills, standardized — target)
      y_gpm:       [10] f32   (standardized — target)
      y_hd:        [10] f32   (standardized — target)
    """

    def __init__(self, hero_ids: np.ndarray, player_feats: np.ndarray,
                 patch_ids: np.ndarray, scalar_inputs: np.ndarray,
        ...
        self.scalar_inputs = torch.tensor(scalar_inputs, dtype=torch.float32).contiguous()
...
def _read_sidecar_tbl(path: Path, mid_set: set[int]):
    """Reads rich-cols sidecar. v5: also needs deaths + assists for kda input."""
    cols = ["match_id", "duration"]
    for p in range(10):
        cols.append(f"p{p}_items")
    aux_cols = ["kills", "deaths", "assists", "gpm", "hero_damage"]
    for p in range(10):
...
def _build_scalar_inputs(side_tbl, keep_idx_side: np.ndarray,
                          scalar_mean_std: tuple[np.ndarray, np.ndarray] | None
                          ) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Compute pre-game-inference-style raw inputs:
      kda_in = log1p((kills + assists) / max(deaths, 1))
      gpm_in = gpm (raw)
      hd_in  = log1p(hero_damage)
    Standardized with train-fit per-feature mean/std.

    Returns (scalar_inputs[N, 10, 3] f32, mu[3] f64, sd[3] f64).
    """
    n_kept = len(keep_idx_side)
    raw = np.empty((n_kept, 10, 3), dtype=np.float32)
    for p in range(10):
        k = side_tbl.column(f"p{p}_kills").to_numpy(zero_copy_only=False)[keep_idx_side].astype(np.float32)
        d = side_tbl.column(f"p{p}_deaths").to_numpy(zero_copy_only=False)[keep_idx_side].astype(np.float32)
        a = side_tbl.column(f"p{p}_assists").to_numpy(zero_copy_only=False)[keep_idx_side].astype(np.float32)
        g = side_tbl.column(f"p{p}_gpm").to_numpy(zero_copy_only=False)[keep_idx_side].astype(np.float32)
        h = side_tbl.column(f"p{p}_hero_damage").to_numpy(zero_copy_only=False)[keep_idx_side].astype(np.float32)
        kda_raw = np.log1p(np.maximum((k + a) / np.maximum(d, 1.0), 0.0))
        gpm_raw = np.nan_to_num(g, nan=0.0, posinf=5000.0, neginf=0.0)
        hd_raw = np.log1p(np.maximum(np.nan_to_num(h, nan=0.0, posinf=500000.0, neginf=0.0), 0.0))
        raw[:, p, 0] = kda_raw
        raw[:, p, 1] = gpm_raw
        raw[:, p, 2] = hd_raw

    if scalar_mean_std is None:
        flat = raw.reshape(-1, 3)
        finite = np.isfinite(flat).all(axis=1)
        mu = flat[finite].mean(axis=0)
        sd = flat[finite].std(axis=0)
        sd = np.where(sd < 1e-6, 1.0, sd)
    else:
        mu, sd = scalar_mean_std
    std = ((raw - mu[None, None, :]) / sd[None, None, :]).astype(np.float32)
    # Clip extreme outliers to [-5, 5] for stability.
    std = np.clip(std, -5.0, 5.0)
    return std, mu.astype(np.float64), sd.astype(np.float64)
...
def _build_foundation_targets(
    ...
    """Same as v4 _build_foundation_targets, but also builds scalar_inputs
    (kda/gpm/hd as inputs) using the same sidecar.

    Returns (hero_ids_kept, pf_kept, y_win_kept, y_dur_log, y_dur_bucket,
             items_per_slot, y_kda, y_gpm, y_hd, scalar_inputs,
             aux_mu, aux_sd, sc_mu, sc_sd).
    """
    ...
    # Item vocab mapping (vectorized) — same as v4.
    _t0 = time.time()
    ...
    if aux_mean_std is None:
        ...
        mu_t = flat.mean(axis=0)
        sd_t = flat.std(axis=0)
        sd_t = np.where(sd_t < 1e-6, 1.0, sd_t)
    else:
        mu_t, sd_t = aux_mean_std
    aux_std_arr = ((aux_raw - mu_t[None, None, :]) / sd_t[None, None, :]).astype(np.float32)
    ...
    # Scalar inputs (v5 new).
    scalar_inputs, sc_mu, sc_sd = _build_scalar_inputs(side_tbl, keep_idx_side,
                                                         scalar_mean_std=scalar_mean_std)
    ...
    if canonical_sort:
        ...
        y_hd = y_hd[rows, perm]
        scalar_inputs = scalar_inputs[rows, perm]
    ...
    return (hero_ids_kept, player_feats_kept, y_win_kept, y_dur_log, y_dur_bucket,
            items_per_slot, y_kda, y_gpm, y_hd, scalar_inputs,
            mu_t.astype(np.float64), sd_t.astype(np.float64),
            sc_mu, sc_sd)
...
def load_train_val(seed: int, n_target: int, feat_names: list[str],
    ...
    v5: no account-id sidecar; new scalar inputs (kda/gpm/hd) loaded from
    rich_cols sidecar.
    """
    ...
    if sidecar_dir is None or vocab_path is None:
        raise SystemExit("v5 requires sidecar_dir and vocab_path.")
    ...
    if smoke and len(h_va_k) == 0 and len(h_tr_k) > 100:
        ...
        sc_in_tr = sc_in_tr[:-n_carve]
        train_patch_ids_kept = train_patch_ids_kept[:-n_carve]

    train_ds = FoundationDataset(h_tr_k, pf_tr_k, train_patch_ids_kept, sc_in_tr,
                                  y_win_tr, y_dur_tr, y_durb_tr, items_tr,
                                  y_kda_tr, y_gpm_tr, y_hd_tr, item_vocab_size)
    val_ds = FoundationDataset(h_va_k, pf_va_k, val_patch_ids_kept, sc_in_va,
    ...
    meta = {
        ...
        "aux_train_mean": mu_t.tolist(),
        "aux_train_std": sd_t.tolist(),
        "scalar_inputs_train_mean": sc_mu.tolist(),
        "scalar_inputs_train_std":  sc_sd.tolist(),
        "scalar_input_names": ["kda_log1p", "gpm_raw", "hd_log1p"],
...
__all__ = ["FoundationDataset", "load_train_val", "assert_no_test_dates",
           ...
           "load_arrays", "stratified_subsample", "N_SCALAR_INPUTS"]
