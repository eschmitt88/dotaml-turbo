"""Three-phase training entry point for v6-jepa-pretrain-finetune-740.
...
Phase 1 (pretrain) runs JEPA-style self-supervised pre-training:
  - Student encoder forward on masked inputs.
  - EMA teacher forward on UN-masked inputs (no_grad).
  - Loss = SmoothL1 between predictor(student_reps) and teacher_reps
    at masked slot positions. One scalar.
  - Per-step EMA update of teacher params.
Writes results/pretrain_encoder.pt + JEPA loss history + collapse
diagnostics + mid-pretrain probe history.
...
def anchors_deltas(val_auc: float, anchors: dict) -> dict:
    ...
    name_map = [
        ...
        ("delta_vs_v5_mid_probe_peak", "v5_mid_probe_peak_val_auc"),
...
def run_pretrain(cfg: dict, splits: dict, smoke: bool) -> int:
    ...
    model = build_model_v6(mhp, vocab_size=int(cfg["hero"]["vocab_size"]),
    ...
    ema = EMATeacherV6(model, momentum=float(pre_cfg["ema_momentum"])).to(device)
    print(f"[pretrain] EMA teacher constructed momentum={pre_cfg['ema_momentum']}")

    # JEPA predictor MLP — separate module, NOT part of the encoder
    # checkpoint. Lives only during Phase 1.
    predictor = JEPAPredictor(d_model=int(model.d_model)).to(device)
    n_pred_params = sum(p.numel() for p in predictor.parameters())
    print(f"[pretrain] JEPA predictor: {n_pred_params:,} params")

    bs = int(pre_cfg["batch_size"])
    train_loader, val_loader = make_loaders(train_ds, val_ds, bs)
    # Optimizer covers student encoder + JEPA predictor. Teacher is frozen.
    ...
    for epoch in range(int(pre_cfg["max_epochs"])):
        ...
        # Collapse-detector running aggregates (averaged across batches).
        rep_l2_running = 0.0
        rep_l2_std_running = 0.0
        pairwise_cos_running = 0.0
        any_collapse_warning_in_epoch = False
        n_seen = 0
        for batch in train_loader:
            ...
            # Per-slot bool mask: True wherever ANY group is masked for the row.
            slot_mask = slot_mask_from_groups(mask_dict, B, device)

            with torch.autocast(device_type=device.type, dtype=autocast_dtype,
                                 enabled=autocast_dtype is not None):
                # Student: encode the MASKED input.
                student_memory, _ = model.encode(
                    hero_ids, pf, items_input=items,
                    scalar_inputs=sc_in, mask_dict=mask_dict,
                )
                # Teacher: encode the UN-MASKED input (mask_dict=None).
                with torch.no_grad():
                    teacher_memory, _ = ema.teacher.encode(
                        hero_ids, pf, items_input=items,
                        scalar_inputs=sc_in, mask_dict=None,
                    )
                loss, n_mask_slots = jepa_loss(
                    predictor, student_memory, teacher_memory, slot_mask,
                )
            ...
            # Collapse diagnostics on a STUDENT rep snapshot (cheap).
            diag = representation_diagnostics(student_memory.detach())
            rep_l2_running += diag["rep_l2_mean"]
            rep_l2_std_running += diag["rep_l2_std"]
            pairwise_cos_running += diag["pairwise_cos_mean"]
            if diag["collapse_warning"]:
                any_collapse_warning_in_epoch = True

            sum_jepa_loss += float(loss.item()) * B
            sum_n_mask_slots += int(n_mask_slots)
            n_batches += 1
            global_step += 1

        ep_dt = time.time() - ep_t0
        avg_jepa = sum_jepa_loss / max(n_seen, 1)
        avg_mask_rate = {g: avg_mask_rate[g] / max(n_batches, 1) for g in PRETRAIN_GROUPS}
        avg_rep_l2 = rep_l2_running / max(n_batches, 1)
        avg_rep_l2_std = rep_l2_std_running / max(n_batches, 1)
        avg_pairwise_cos = pairwise_cos_running / max(n_batches, 1)
        ep_summary = (f"[pretrain] epoch {epoch+1}/{pre_cfg['max_epochs']}  "
                      f"jepa_loss={avg_jepa:.4f}  "
                      f"lr={cur_lr:.2e}  ({ep_dt:.1f}s)")
        print(ep_summary)
        print("    mask rates: " +
              " ".join(f"{g}={avg_mask_rate[g]:.3f}" for g in PRETRAIN_GROUPS))
        print(f"    rep_l2_mean={avg_rep_l2:.4f} rep_l2_std={avg_rep_l2_std:.4f} "
              f"pairwise_cos_mean={avg_pairwise_cos:.4f} "
              f"collapse_warning={any_collapse_warning_in_epoch} "
              f"masked_slots/batch={sum_n_mask_slots/max(n_batches,1):.1f}")

        ep_record = {
            "epoch": epoch + 1,
            "jepa_loss": float(avg_jepa),
            "per_group_mask_rate": avg_mask_rate,
            "n_masked_slots_per_batch_mean": float(sum_n_mask_slots / max(n_batches, 1)),
            "rep_l2_mean": float(avg_rep_l2),
            "rep_l2_std": float(avg_rep_l2_std),
            "pairwise_cos_mean": float(avg_pairwise_cos),
            "collapse_warning": bool(any_collapse_warning_in_epoch),
            "lr": float(cur_lr),
