...
def main() -> int:
    ...
    ap.add_argument("--ablation", required=True,
                    choices=["transformer_plus_features"])
    ...
    # Resolve ablation spec. NOTE: this experiment's config nests the Transformer
    # blocks under transformer_* keys to coexist with the LightGBM blocks (model:, smoke:, ablations:).
    ...
    # Train.
    opt = cfg["transformer_optim"]
    max_epochs = int(opt["max_epochs"]) if not args.smoke else int(cfg["transformer_smoke"]["max_epochs"])
    ...
    parent_anchor = float(anchors.get(
    ...
    # Equality-band check vs the extended-740 ceiling.
    ...
    if not args.smoke:
        ...
        equality_band_check = {
            ...
            "delta_vs_parent": float(tr.best_val_auc - parent_anchor),
            "equality_band_holds": in_band,
        }
        verdict = "HOLDS (within ±0.001 of 0.6477)" if in_band else "BROKEN"
        print(f"EQUALITY-BAND CHECK: {verdict} (val_auc={tr.best_val_auc:.4f}, "
    ...
    metrics = {
        ...
        "delta_combined_vs_transformer_plus_features_extended_740": tr.best_val_auc - parent_anchor,
        "delta_combined_vs_proposal_band_lower": tr.best_val_auc - band_lo,
        "delta_combined_vs_proposal_band_upper": tr.best_val_auc - band_hi,
        "equality_band_holds": equality_band_check["equality_band_holds"] if equality_band_check else None,
